#!/bin/bash

set -e

HERE=`dirname $0`
HERE=`realpath $HERE`
echo $HERE
source $HERE/settings.sh

SKIP_SPACK=0     # skip installation of spack
SKIP_MOCHI=0     # skip installation of mochi repo
SKIP_MONA=0      # skip installation of colza

while [[ $# -gt 0 ]]
do
    case $1 in
    --skip-spack)
        SKIP_SPACK=1
        shift
        ;;
    --skip-mochi)
        SKIP_MOCHI=1
        shift
        ;;
    --skip-mona)
        SKIP_MONA=1
        shift
        ;;
    *)
        echo "====> ERROR: unkwown argument $1"
        exit -1
        ;;
    esac
done

function install_spack {
    # Checking if spack is already there
    if [ -d $MONA_SPACK_LOCATION ];
    then
        echo "====> ERROR: spack is already installed in $MONA_SPACK_LOCATION," \
             "please remove it or use --skip-spack"
        exit -1
    fi
    # Cloning spack and getting the correct release tag
    echo "====> Cloning Spack"
    git clone https://github.com/spack/spack.git $MONA_SPACK_LOCATION
}

function setup_spack {
    echo "====> Setting up spack"
    . $MONA_SPACK_LOCATION/share/spack/setup-env.sh
}

function install_mochi {
    if [ -d $MONA_MOCHI_LOCATION ]; then
        echo "====> ERROR: Mochi already installed in $MONA_MOCHI_LOCATION," \
             " please remove it or use --skip-mochi"
        exit -1
    fi
    echo "====> Cloning Mochi namespace"
    git clone https://github.com/mochi-hpc/mochi-spack-packages.git $MONA_MOCHI_LOCATION
}

function install_mona {
    echo "====> Setting up environment"
    spack env create $MONA_SPACK_ENV $HERE/spack.yaml
    echo "====> Activating environment"
    spack env activate $MONA_SPACK_ENV
    echo "====> Adding Mochi namespace"
    spack repo add --scope env:$MONA_SPACK_ENV $MONA_MOCHI_LOCATION
    echo "====> Installing"
    spack install
    spack env deactivate
}

if [ "$SKIP_SPACK" -eq "0" ]; then
    install_spack
fi

if [ "$SKIP_MOCHI" -eq "0" ]; then
    install_mochi
fi

setup_spack

if [ "$SKIP_MONA" -eq "0" ]; then
    install_mona
fi
